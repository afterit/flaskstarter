...
def plug_extension(name: str):
    ...
    settings = toml.load(
        os.path.join(os.getcwd(), get_app_dir(), "settings.toml")
    )
    if (
        ...
        click.echo(
            "An extension with such a name seems to be already plugged."
        )
        exit(0)
    ...
    with open(
        os.path.join(os.getcwd(), get_app_dir(), "settings.toml"), "w"
    ) as f:
