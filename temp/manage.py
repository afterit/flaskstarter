...
@click.group()
def manage():
    '''This script manages the temp project.'''
...
def runserver(port):
    ...
    cmd = ''
    if os.name == 'posix':
        cmd = f'. .venv/bin/activate; export FLASK_APP=temp; export FLASK_ENV=development; flask run --port={port}'
    elif os.name == 'nt':
        cmd = f'call .venv/Scripts/activate; set FLASK_APP=temp; set FLASK_ENV=development; flask run --port={port}'
...
def create_blueprint(name : str):
    ...
    os.mkdir(os.path.join(os.getcwd(), 'temp', name))
    blueprint = open(os.path.join(os.getcwd(), 'temp', 'blueprints', f'{name}.py'), 'w')
